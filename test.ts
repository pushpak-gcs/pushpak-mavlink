...
mavlink.on("mavlink:error", (event: any) => {
  console.error(" MAVLink error:", event.error);
});
  ...
  switch (event.messageName) {
    case "ATTITUDE":
      ...
      if (Date.now() - lastPrintTime > 2000) {
        console.log(` Attitude: roll=${(event.payload.roll * 57.2958).toFixed(1)}° pitch=${(event.payload.pitch * 57.2958).toFixed(1)}° yaw=${(event.payload.yaw * 57.2958).toFixed(1)}°`);
      }
      break;
